...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	switch (type)
	{
		...
		case DAMAGE_TYPE_NORMAL_RANGE:
			...
			if (!IsNPC() && pAttacker->GetJob() == JOB_ASSASSIN && CPVPManager::instance().IsSpecialDuel(pAttacker, this))
			{
				dam = 1000;
			}			
			else if (!IsNPC() && pAttacker->GetJob() == JOB_WARRIOR && CPVPManager::instance().IsSpecialDuel(pAttacker, this))
			{
				dam = 1100;
			}			
			else if (!IsNPC() && pAttacker->GetJob() == JOB_SURA && CPVPManager::instance().IsSpecialDuel(pAttacker, this))
			{
				dam = 1100;
			}			
			else if (!IsNPC() && pAttacker->GetJob() == JOB_SHAMAN && CPVPManager::instance().IsSpecialDuel(pAttacker, this))
			{
				dam = 1100;
			}
